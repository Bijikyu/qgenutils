...
const advancedValidation = require('../../lib/advanced-validation');

describe('Advanced Validation Utilities', () => {
  describe('validateEmail', () => {
    test('should validate correct email formats', () => {
      expect(advancedValidation.validateEmail('user@example.com')).toBe('');
      expect(advancedValidation.validateEmail('test.email@domain.co.uk')).toBe('');
      expect(advancedValidation.validateEmail('user+tag@subdomain.example.org')).toBe('');
      expect(advancedValidation.validateEmail('simple@test.io')).toBe('');
    });

    test('should reject invalid email formats', () => {
      expect(advancedValidation.validateEmail('invalid-email')).toBe('Please enter a valid email address');
      expect(advancedValidation.validateEmail('user@')).toBe('Please enter a valid email address');
      expect(advancedValidation.validateEmail('@domain.com')).toBe('Please enter a valid email address');
      expect(advancedValidation.validateEmail('user@domain')).toBe('Please enter a valid email address');
      expect(advancedValidation.validateEmail('user.domain.com')).toBe('Please enter a valid email address');
    });

    test('should handle empty or invalid input types', () => {
      expect(advancedValidation.validateEmail('')).toBe('Email address is required');
      expect(advancedValidation.validateEmail('   ')).toBe('Email address is required');
      expect(advancedValidation.validateEmail(null)).toBe('Email address is required');
      expect(advancedValidation.validateEmail(undefined)).toBe('Email address is required');
      expect(advancedValidation.validateEmail(123)).toBe('Email address is required');
    });

    test('should sanitize input before validation', () => {
      expect(advancedValidation.validateEmail('  user@example.com  ')).toBe('');
      expect(advancedValidation.validateEmail('user@example.com\x00')).toBe('');
    });
  });

  describe('validateRequired', () => {
    test('should validate non-empty required fields', () => {
      expect(advancedValidation.validateRequired('John', 'Name')).toBe('');
      expect(advancedValidation.validateRequired('Valid content', 'Description')).toBe('');
      expect(advancedValidation.validateRequired('abc', 'Username', 3)).toBe('');
    });

    test('should reject empty or whitespace-only fields', () => {
      expect(advancedValidation.validateRequired('', 'Name')).toBe('Name is required');
      expect(advancedValidation.validateRequired('   ', 'Title')).toBe('Title is required');
      expect(advancedValidation.validateRequired('\t\n', 'Content')).toBe('Content is required');
    });

    test('should validate minimum length requirements', () => {
      expect(advancedValidation.validateRequired('ab', 'Username', 3)).toBe('Username must be at least 3 characters long');
      expect(advancedValidation.validateRequired('a', 'Password', 8)).toBe('Password must be at least 8 characters long');
      expect(advancedValidation.validateRequired('valid', 'Title', 5)).toBe('');
    });

    test('should handle invalid input types', () => {
      expect(advancedValidation.validateRequired(null, 'Field')).toBe('Field is required');
      expect(advancedValidation.validateRequired(undefined, 'Field')).toBe('Field is required');
      expect(advancedValidation.validateRequired(123, 'Field')).toBe('Field is required');
    });

    test('should use singular/plural correctly in error messages', () => {
      expect(advancedValidation.validateRequired('', 'Field', 1)).toBe('Field must be at least 1 character long');
      expect(advancedValidation.validateRequired('a', 'Field', 5)).toBe('Field must be at least 5 characters long');
    });
  });
  ...
  describe('combineValidations', () => {
    ...
    test('should work with real validation functions', () => {
      ...
      const result = advancedValidation.combineValidations(
        () => advancedValidation.validateRequired(username, 'Username', 3),
        () => advancedValidation.validateEmail(email)
      );
