...
function ensureProtocol(url) {
  console.log(`ensureProtocol is running with ${url}`); // show raw input for troubleshooting
  try {
    // Validate that input is a usable string before processing
    if (typeof url !== 'string' || !url) {
      qerrors(new Error('invalid url input'), 'ensureProtocol', url); // record bad input
      console.log(`ensureProtocol is returning null`); // exit due to invalid value
    ...
    if (trimmedUrl.length === 0) {
      console.log(`ensureProtocol is returning null`); // exit due to empty string
    ...
    console.log(`ensureProtocol is returning ${finalUrl}`); // show normalized URL
    return finalUrl;
  } catch (error) {
    // Handle unexpected errors in URL processing
    qerrors(error, 'ensureProtocol', url); // capture error details
...
function normalizeUrlOrigin(url) {
  console.log(`normalizeUrlOrigin is running with ${url}`); // show URL before normalization
  try {
    ...
    if (processedUrl === null) {
      console.log(`normalizeUrlOrigin is returning null`); // abort due to invalid input
      return null;
    }
    ...
    console.log(`normalizeUrlOrigin is returning ${normalizedOrigin}`); // show normalized origin
...
function stripProtocol(url) {
  console.log(`stripProtocol is running with ${url}`); // record URL before strip
  try {
    ...
    console.log(`stripProtocol is returning ${processed}`); // show stripped URL
    return processed;
  } catch (error) {
    // Handle unexpected errors in string processing
    qerrors(error, 'stripProtocol', url); // record failure to strip
...
function parseUrlParts(url) {
  console.log(`parseUrlParts is running with ${url}`); // show full URL to parse
  try {
    ...
    // If protocol normalization failed, abort parsing
    if (processedUrl === null) {
      console.log(`parseUrlParts is returning null`); // abort due to invalid input
      return null;
    }
    ...
    console.log(`parseUrlParts is returning ${JSON.stringify(result)}`); // show parsed parts
    return result;
  } catch (error) {
    // Handle URLs that can't be parsed by URL constructor
    qerrors(error, 'parseUrlParts', url); // record parsing error
    return null; // Return null to indicate parsing failure
  }
}
