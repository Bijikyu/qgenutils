...
function hasProtocol(url) {
  return /^https?:\/\//i.test(url); // check for http or https prefix
}
...
function ensureProtocol(url) {
  console.log(`ensureProtocol is running with ${url}`); // record original URL value
  try {
    // Validate that input is a usable string before processing
    if (typeof url !== 'string' || !url) {
      qerrors(new Error('invalid url input'), 'ensureProtocol', url); // capture invalid parameter
      console.log(`ensureProtocol is returning null`); // nothing to process
      return null; // signal invalid input
    }

    // Handle empty or invalid input gracefully - return null for missing URLs
    if (!url || url.trim().length === 0) {
      console.log(`ensureProtocol is returning null`); // empty string or whitespace
      return null; // invalid or missing URL
    }

    let finalUrl = url; // work with copy so caller input untouched

    // Check if protocol is already present using centralized detection
    if (!hasProtocol(finalUrl)) {
      finalUrl = 'https://' + finalUrl; // prepend https when protocol absent
    }

    console.log(`ensureProtocol is returning ${finalUrl}`); // show completed URL
    return finalUrl; // return normalized URL
  } catch (error) {
    // Handle unexpected errors in URL processing
    qerrors(error, 'ensureProtocol', url); // record exception with context
...
function normalizeUrlOrigin(url) {
  console.log(`normalizeUrlOrigin is running with ${url}`); // incoming URL to normalize
  try {
    // First ensure the URL has a protocol, then extract and normalize origin
    const processedUrl = ensureProtocol(url); // guarantee we have protocol before parsing

    // If protocol normalization failed, abort parsing
    if (processedUrl === null) {
      console.log(`normalizeUrlOrigin is returning null`); // invalid URL provided
      return null; // abort when cannot process
    }

    // Parse the URL to extract components
    // This allows us to normalize the origin while preserving other parts
    const urlObj = new URL(processedUrl); // parse with WHATWG URL

    // Build normalized origin, preserving explicit ports (including 443 for HTTPS)
    let normalizedOrigin = `${urlObj.protocol}//${urlObj.hostname.toLowerCase()}`; // base origin string
    if (urlObj.port && urlObj.port !== '') {
      normalizedOrigin += `:${urlObj.port}`; // include explicit port when present
    }

    console.log(`normalizeUrlOrigin is returning ${normalizedOrigin}`); // show canonical form
    return normalizedOrigin; // return normalized origin
  } catch (error) {
    // Handle malformed URLs that can't be parsed
    qerrors(error, 'normalizeUrlOrigin', url); // record parsing failure
...
function stripProtocol(url) {
  console.log(`stripProtocol is running with ${url}`); // show raw URL before modifications
  try {
    ...
    console.log(`stripProtocol is returning ${processed}`); // show cleaned URL
    return processed; // return stripped value
  } catch (error) {
    // Handle unexpected errors in string processing
    qerrors(error, 'stripProtocol', url); // log details for debugging
    return url; // fallback to original when regex fails
  }
}
...
function parseUrlParts(url) {
  console.log(`parseUrlParts is running with ${url}`); // note original URL value
  try {
    // First normalize the URL to ensure it has a protocol for valid parsing
    const processedUrl = ensureProtocol(url); // add protocol if missing

    // If protocol normalization failed, abort parsing
    if (processedUrl === null) {
      console.log(`parseUrlParts is returning null`); // cannot parse invalid URL
      return null; // indicate failure
    }

    // Parse URL into components using native URL constructor
    const parsed = new URL(processedUrl); // parse standardized URL

    // Create structured result with base URL and endpoint
    const result = {
      baseUrl: parsed.origin,                    // base address for request
      endpoint: parsed.pathname + parsed.search  // include path and query
    };

    console.log(`parseUrlParts is returning ${JSON.stringify(result)}`); // output structured info
    return result; // provide parsed components
  } catch (error) {
    // Handle URLs that can't be parsed by URL constructor
    qerrors(error, 'parseUrlParts', url); // log failure in detail
