...
function ensureProtocol(url) {
  ...
  try {
    ...
    // Check if protocol is already present (case-insensitive)
    const hasProto = /^https?:\/\//i.test(finalUrl);

    // Add HTTPS protocol if none present (security default)
    if (!hasProto) { 
...
function normalizeUrlOrigin(url) {
  ...
  try {
    ...
    // Extract and normalize the origin (protocol + hostname + port if not default)
    let origin = `${parsedUrl.protocol}//${parsedUrl.hostname}`;
    ...
    console.log(`normalizeUrlOrigin is returning ${origin.toLowerCase()}`);
...
function stripProtocol(url) {
  ...
  try {
    // Chain regex replacements to remove protocol and trailing slash
    const processed = url
