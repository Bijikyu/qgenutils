...
function isValidObject(obj) {
  return obj !== null && obj !== undefined && typeof obj === 'object' && !Array.isArray(obj); // ensure plain object
}
...
function isValidString(str) {
  return typeof str === 'string' && str.trim().length > 0; // reject empty or non-string values
}
...
function hasMethod(obj, methodName) {
  try {
    return obj && typeof obj[methodName] === 'function'; // confirm property is callable
  } catch (error) {
    qerrors(error, 'hasMethod', { obj: typeof obj, methodName }); // record failure context
    return false; // safely indicate absence of method
  }
}
...
function isValidExpressResponse(res) {
  return hasMethod(res, 'status') && hasMethod(res, 'json'); // both Express methods must exist
}
