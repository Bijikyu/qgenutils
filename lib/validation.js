 ...
 */
function requireFields(res, obj, requiredFields) {
  console.log(`requireFields is running with ${requiredFields.join(',')}`); // Log validation attempt with field list
  try {
    ...
    if (missingFields.length > 0) {
      // Create user-friendly error message listing all missing fields
      const errorMessage = `Missing required fields: ${missingFields.join(', ')}`;
      
      // Send standardized 400 Bad Request response with specific field information
      // Using status 400 because this is a client error (bad request format)
      res.status(400).json({ error: errorMessage });
      
      console.log(`requireFields is returning false`); // Log validation failure
      return false; // Signal to calling code that validation failed
    }
