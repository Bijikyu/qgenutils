...
const { sendValidationError, sendServerError } = require('./response-utils');
...
function requireFields(obj, requiredFields, res) {
  console.log(`requireFields is running with ${requiredFields ? requiredFields.join(',') : 'undefined'}`); // which fields are expected
  try {
    // Validate input parameters
    if (!requiredFields || !Array.isArray(requiredFields)) {
      console.log(`requireFields is returning false - invalid requiredFields parameter`); // developer mis-use
      sendServerError(res, 'Internal validation error'); // generic error to client
      return false; // indicate failure
    }
    
    if (!isValidObject(obj)) {
      console.log(`requireFields is returning false - invalid obj parameter`); // input is not an object
      sendServerError(res, 'Internal validation error'); // respond with server error
    ...
    const missingFields = requiredFields.filter(field => !obj[field]); // collect every field that is absent or falsy
    ...
    if (missingFields.length > 0) {
      ...
      sendValidationError(res, 'Missing required fields', { missing: missingFields });

      console.log(`requireFields is returning false`); // indicate failure due to missing fields
      return false; // signal validation failure
    }

    // All required fields are present and truthy
    console.log(`requireFields is returning true`); // all checks passed
    return true; // signal success
  } catch (error) {
    // Handle unexpected errors during validation (malformed objects, etc.)
    qerrors(error, 'requireFields', { requiredFields, obj }); // log stack trace and data

    // Send generic server error to client (don't expose internal details)
    sendServerError(res, 'Internal validation error', error, 'requireFields'); // reuse centralized helper

    return false; // fail closed on unexpected error
  }
}
