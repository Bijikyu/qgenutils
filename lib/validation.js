 ...
 * This centralizes the error response pattern used throughout validation
 * 
 ...
 */
function sendValidationError(res, statusCode, message, additionalData = {}) {
...
function requireFields(obj, requiredFields, res) {
  ...
  try {
    ...
    if (!requiredFields || !Array.isArray(requiredFields)) {
      ...
      sendValidationError(res, 500, 'Internal validation error');
      return false;
    }
    ...
    if (missingFields.length > 0) {
      ...
      sendValidationError(res, 400, 'Missing required fields', { missing: missingFields });
