...
function requireFields(obj, requiredFields, res) {
  console.log(`requireFields is running with ${requiredFields && Array.isArray(requiredFields) ? requiredFields.join(',') : 'undefined'}`); // log invocation with field list
  try { // start validation
    if (!requiredFields || !Array.isArray(requiredFields)) { // verify requiredFields parameter
      console.log(`requireFields is returning false - invalid requiredFields parameter`); // trace failure reason
      sendServerError(res, 'Internal validation error'); // inform client of configuration issue
      return false; // stop processing on invalid parameters
    }

    if (!isValidObject(obj)) { // ensure provided object is valid
      console.log(`requireFields is returning false - invalid obj parameter`); // trace object validation failure
      sendServerError(res, 'Internal validation error'); // unified server error response
    ...
    if (missingFields.length > 0) { // if any field is missing respond immediately
      ...
      console.log(`requireFields is returning false`); // record validation failure
      return false; // signal failed validation
    }

    console.log(`requireFields is returning true`); // record successful validation
    return true; // validation passed
  } catch (error) {
    qerrors(error, 'requireFields', { requiredFields, obj }); // log unexpected error

    sendServerError(res, 'Internal validation error', error, 'requireFields'); // generic server error response
