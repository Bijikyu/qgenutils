...
const { qerrors } = require('qerrors'); // error logging
const { isValidObject } = require('./input-validation'); // basic object check
const { sendValidationError, sendServerError } = require('./response-utils'); // standardized error responses
...
function requireFields(obj, requiredFields, res) {
  console.log(`requireFields is running with ${requiredFields && Array.isArray(requiredFields) ? requiredFields.join(',') : 'undefined'}`); // Log validation attempt with field list
  try {
    // Validate input parameters
    if (!requiredFields || !Array.isArray(requiredFields)) {
      console.log(`requireFields is returning false - invalid requiredFields parameter`);
      sendServerError(res, 'Internal validation error');
      return false;
    }
    ...
    // This prevents multiple round-trips where users fix one field at a time
    const missingFields = requiredFields.filter(field => !obj[field]);

    // If any fields are missing, send detailed error response and return false
    // The immediate error response prevents further processing with incomplete data
    if (missingFields.length > 0) {
      // Send standardized error response with detailed field information
      // Including the specific missing fields helps developers and UI to highlight exact issues
      sendValidationError(res, 'Missing required fields', { missing: missingFields });

      console.log(`requireFields is returning false`); // Log validation failure with context
      return false; // Signal validation failure to calling code
    }

    // All required fields are present and truthy
    console.log(`requireFields is returning true`); // Log validation success
    return true; // Signal to calling code that validation passed and processing can continue
  } catch (error) {
    ...
    sendServerError(res, 'Internal validation error', error, 'requireFields');
