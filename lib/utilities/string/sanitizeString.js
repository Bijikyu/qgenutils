...
function sanitizeString(input) {
  ...
  try {
    ...
    sanitized = sanitized.replace(/(?:javascript|data|vbscript|file|ftp|chrome|chrome-extension):/gi, '');
    
    // Remove event handlers (comprehensive list)
    sanitized = sanitized.replace(/\bon\w+\s*=/gi, '');
    
    // Remove dangerous attributes
    sanitized = sanitized.replace(/\b(?:style|src|href|action|formaction|background|poster)\s*=/gi, '');
    
    // Remove control characters except tab, newline, and carriage return
    sanitized = sanitized.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
    
    // Remove potentially dangerous escape sequences (enhanced)
    sanitized = sanitized.replace(/\\[ux][\da-fA-F]{2,8}/gi, '');
    
    // Remove base64 data schemes that could contain malicious content
    sanitized = sanitized.replace(/data:(?:text\/|application\/|image\/)[^;]*;base64,[a-zA-Z0-9+/=]+/gi, '');
