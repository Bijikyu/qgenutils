...
const sanitizeString = (input) => {
  ...
  try {
    if (input == null) return logger.debug(`sanitizeString: null or undefined input`), ``;
    let str;
    if (typeof input === 'string') str = input;
    else if (typeof input === 'number' || typeof input === 'boolean') str = String(input);
    else return logger.warn(`sanitizeString: rejecting complex object input for security`, { inputType: typeof input }), ``;
    if (str === ``) return logger.debug(`sanitizeString: empty string input`), ``;
    const sanitized = sanitizeHtml(str, { allowedTags: [], allowedAttributes: {}, textFilter: text => text, disallowedTagsMode: 'discard', enforceHtmlBoundary: false });
