 ...
 */
const groupBy = (array, keyFn) => (!Array.isArray(array) || typeof keyFn !== 'function') ? (typeof keyFn !== 'function' ? (() => { throw new Error('groupBy requires a key function'); })() : {}) : array.reduce((groups, item) => {const key = keyFn(item);!groups[key] && (groups[key] = []);groups[key].push(item);return groups;}, {});
