...
function scheduleInterval(callback, intervalMs, options = {}) {
  ...
  const jobId = identifier || `interval_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

  const executeCallback = async () => { // wrapper to handle async execution
    if (cancelled) return;
...
module.exports = scheduleInterval;
