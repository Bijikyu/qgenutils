...
const lagNs: bigint = end - start; // keep as BigInt
  // Convert to milliseconds with enhanced bounds checking - prevent overflow
  ...
  // Additional safety check before Number conversion
  if (clampedLagNs > maxSafeBigInt) {
    callback(60000); // Return max safe value (60 seconds)
    return;
  }
