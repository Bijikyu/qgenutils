...
function collectSystemMetrics(state) {
  const now = Date.now();
  const lastCpuUsage = state.lastCpuUsage || process.cpuUsage();
  const lastCollectionTime = state.lastCollectionTime || now;
  ...
  // CPU metrics
  const elapsedMs = Math.max(1, now - lastCollectionTime);
  const currentCpuUsage = process.cpuUsage(lastCpuUsage);
  const totalCpuTime = currentCpuUsage.user + currentCpuUsage.system;
  const cpuUsage = Math.min(100, (totalCpuTime / (elapsedMs * 1000)) * 100);
...
function calculateApplicationMetrics(state) {
  const { responseTimes, requestTimestamps } = state;
  const now = Date.now();
  ...
  const requestRate = throughput / 60;
...
function createMetricsCollector(_options = {}) {
  const _maxResponseTimes = 100;
  const _maxRequestTimestamps = 500;
  ...
  return {
     ...
     */
    collect() {
      const systemMetrics = collectSystemMetrics(state.getAll());
      const appMetrics = calculateApplicationMetrics(state.getAll());
