 ...
 * @example
 * const alerts = analyzePerformanceMetrics({ eventLoopLag: 50, cpuUsage: 90, ... });
...
function analyzePerformanceMetrics(metrics, thresholds = {}, requestCount = 0) { // analyze metrics and generate alerts
  ...
  const config = { ...DEFAULT_THRESHOLDS, ...thresholds }; // merge with defaults
  const alerts = []; // collect alerts
  const timestamp = Date.now(); // alert timestamp
  ...
  const hasRequests = requestCount > 0 || (Array.isArray(metrics.requestTimestamps) && metrics.requestTimestamps.length > 0); // check if any requests recorded
...
module.exports = analyzePerformanceMetrics;
