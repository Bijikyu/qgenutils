 ...
 * @example
 * const health = getPerformanceHealthStatus(metrics, alerts);
...
function getPerformanceHealthStatus(metrics, alerts = [], alertLimit = 10) { // compute overall health status
  ...
  const recentAlerts = alerts.slice(-alertLimit); // get most recent alerts
  const criticalAlerts = recentAlerts.filter(alert => alert.severity === 'critical'); // filter critical
  const warningAlerts = recentAlerts.filter(alert => alert.severity === 'warning'); // filter warnings

  let status = 'healthy'; // default status
  const recommendations = []; // actionable recommendations
  ...
  if (criticalAlerts.length > 0) { // critical alerts take precedence
    ...
    const alertTypes = new Set(criticalAlerts.map(a => a.type)); // get unique alert types
...
module.exports = getPerformanceHealthStatus;
