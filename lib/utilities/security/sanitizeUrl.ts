...
function sanitizeUrl(url: any, options: SanitizeUrlOptions = {}) { // remove sensitive URL params
  ...
  const additionalParams: any = options.additionalParams || [];
  const allSensitiveParams: any = [...SENSITIVE_PARAMS, ...additionalParams];

  try {
    const urlObj: any = new URL(url, 'http://localhost'); // parse URL
