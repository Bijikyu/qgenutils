 ...
 * @example
 * const safePath = validateAndNormalizePath('uploads/image.png');
...
function validateAndNormalizePath(inputPath, options = {}) { // validate path against traversal attacks
  ...
  const allowLeadingSlash = options.allowLeadingSlash || false;

  if (!inputPath || typeof inputPath !== 'string') {
    throw new Error('Invalid path: must be a non-empty string');
  }

  const sanitizedPath = inputPath.replace(/[\x00-\x1F\x7F]/g, ''); // remove null bytes and control chars
  ...
  const normalizedPath = path.normalize(sanitizedPath); // normalize using Node.js path
...
module.exports = validateAndNormalizePath;
