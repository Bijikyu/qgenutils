 ...
 */
const maskStringCore = (value, options = {}) => {
  ...
  const masked = maskChar.repeat(Math.min(maskedLength, maxMaskLength));
 ...
 */
const maskSensitiveValue = (value, key, options = {}) => {
  // Input validation
 ...
 */
const isSensitiveKey = (key, customSensitiveKeys = SENSITIVE_KEYS) => {
  if (typeof key !== 'string') {
    return false;
  }
