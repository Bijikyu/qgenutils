...
function createIpTracker(config = {}) { // factory for IP tracking system
  const maxTrackedIps = config.maxTrackedIps || SECURITY_CONFIG.MAX_TRACKED_IPS;
  const cleanupIntervalMs = config.cleanupIntervalMs || SECURITY_CONFIG.CLEANUP_INTERVAL_MS;
  const blockDurationMs = config.blockDurationMs || SECURITY_CONFIG.BLOCK_DURATION_MS;
  const maxFailedAttempts = config.maxFailedAttempts || SECURITY_CONFIG.MAX_FAILED_ATTEMPTS;
  const windowMs = config.windowMs || SECURITY_CONFIG.RATE_LIMIT_WINDOW_MS;

  const ipData = new Map(); // IP -> tracking data
  const blockedIps = new Map(); // IP -> block expiry timestamp
  ...
  function cleanup() { // cleanup expired entries
    ...
    blockedIps.forEach((expires, ip) => { // remove expired blocks
      if (expires <= now) blockedIps.delete(ip);
    });

    ipData.forEach((data, ip) => { // remove stale IP data
    ...
    if (ipData.size > maxTrackedIps) { // enforce memory limit
      ...
      const toRemove = sorted.slice(0, ipData.size - maxTrackedIps);
...
module.exports = createIpTracker;
