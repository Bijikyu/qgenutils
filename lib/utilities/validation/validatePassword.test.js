...
describe('validatePassword', () => {
  ...
  it('should classify strength based on error count', () => {
    const weak = validatePassword('Password1');
    expect(weak.strength).toBe('weak');

    const veryWeak = validatePassword('abc');
    expect(veryWeak.strength).toBe('very_weak');
  });

  it('should handle invalid input', () => {
    expect(validatePassword(null).isValid).toBe(false);
    expect(validatePassword(null).errors).toContain('invalid_input');
    expect(validatePassword(null).strength).toBe('invalid');
