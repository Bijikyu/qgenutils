...
import { isValid, parseISO, isAfter, isBefore, differenceInDays } from 'date-fns'; // date manipulation library
...
function validateDateRange(startDate, endDate) { // comprehensive date range validation using date-fns
  ...
  const isChronological: any = isBefore(start, end) || start.getTime() === end.getTime(); // start must be before or equal to end

  if (!isChronological) { // reject non-chronological ranges
    return false;
  }

  const isReasonable: any = differenceInDays(end, start) <= 365; // maximum 1 year range for performance

  if (!isReasonable) { // reject excessively long ranges
    return false;
  }

  const isNotFuture: any = !isAfter(end, new Date()); // ensure end date is not in future
