 ...
 */
const validateBooleanCore = (value, options = {}) => {
  const { allowStringBooleans = true, defaultValue = null } = options;
...
const validateBoolean = (value, fieldName) => {
  const result = validateBooleanCore(value, { allowStringBooleans: false });
...
async function validateBooleanField(req, res, fieldName, handler, defaultValue = false) {
  ...
  const result = validateBooleanCore(value, { defaultValue });
