 ...
 * @example
 * const age: any = await validateNumberInRange(req, res, 'age', { min: 0, max: 120 }, handler);
 */
async function validateNumberInRange(req, res, fieldName, options = {}, handler) { // validate number in range
  const { min, max, required = false }: any = options;
  ...
  const numValue: any = Number(value); // convert to number
