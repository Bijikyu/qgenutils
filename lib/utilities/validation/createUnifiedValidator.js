 ...
 * @example
 * const numberValidator = createUnifiedValidator((value, options) => {
 ...
 * });
 */
function createUnifiedValidator(coreValidator, options = {}) {
  const { 
    errorMessages = {},
    transformValue,
   ...
   */
  const validateCore = (value, validateOptions = {}) => {
   ...
   */
  const validateSimple = (value, fieldName, validateOptions = {}) => {
    const result = validateCore(value, validateOptions);
   ...
   */
  const validateMiddleware = async (req, res, fieldName, validateOptions = {}, errorHandler) => {
   ...
   */
  const validateBatch = (data, fieldConfigs) => {
    const results = {};
    const errors = {};
    let isValid = true;
    ...
    for (const [fieldName, config] of Object.entries(fieldConfigs)) {
      ...
      const result = validateCore(value, config);
 ...
 */
function createStandardUnifiedValidator(validationFn, defaultOptions = {}) {
...
function createTypeUnifiedValidator(expectedType, options = {}) {
  ...
  const checkFn = typeCheckMap[expectedType];
  if (!checkFn) {
    throw new Error(`Unsupported type: ${expectedType}`);
  }

  return createStandardUnifiedValidator((value, validateOptions) => {
    const { required = false, defaultValue = null } = validateOptions;
