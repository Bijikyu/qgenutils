...
function parseAndValidateJson(jsonString, expectedType = 'object', defaultValue = null) {
  const parsed: any = safeJsonParse(jsonString, defaultValue);
  
  if (parsed === defaultValue) {
    return defaultValue;
  }
  
  const expectedTypes: any = Array.isArray(expectedType) ? expectedType : [expectedType];
  const actualType: any = Array.isArray(parsed) ? 'array' : typeof parsed;
