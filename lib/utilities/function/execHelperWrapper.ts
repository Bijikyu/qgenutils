...
const execHelperWrapper = (fn: any, options: any = {}): any => {
  ...
  return async function wrappedFunction(...args) {
    const startTime: any = logPerformance ? Date.now() : null;
...
const executeWithTimeout = async (fn: any, args: any, timeoutMs: any): Promise<any> => {
...
const executeWithRetry = async (fn: any, args: any, retryCount: any, retryDelay: any): Promise<any> => {
  let lastError;
...
export default execHelperWrapper;
