...
function normalizeUrlOrigin(url) {
  ...
  try {
    ...
    const protocol: any = urlObj.protocol; // Includes the trailing colon
    const hostname: any = urlObj.hostname.toLowerCase(); // Convert to lowercase
    const port: any = urlObj.port; // Empty string for default ports
...
export default normalizeUrlOrigin;
