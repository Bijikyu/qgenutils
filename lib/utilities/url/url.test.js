const { default: ensureProtocol } = require('./ensureProtocol');
const { default: stripProtocol } = require('./stripProtocol');
const { default: normalizeUrlOrigin } = require('./normalizeUrlOrigin');
const { default: parseUrlParts } = require('./parseUrlParts');

describe('URL Utilities', () => {
  describe('ensureProtocol', () => {
    test('should add https to URLs without protocol', () => {
      expect(ensureProtocol('example.com').processed).toBe('https://example.com');
      expect(ensureProtocol('www.example.com').processed).toBe('https://www.example.com');
    });

    test('should preserve existing protocols', () => {
      expect(ensureProtocol('https://example.com').processed).toBe('https://example.com');
      expect(ensureProtocol('http://example.com').processed).toBe('http://example.com');
    });
  });
