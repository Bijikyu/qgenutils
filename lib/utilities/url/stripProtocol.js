...
function stripProtocol(url) {
  console.log(`stripProtocol is running with ${url}`);
  logger.debug(`stripProtocol is running with ${url}`);
  
  try {
    // Validate input
    if (!isValidString(url)) {
      console.log(`stripProtocol returning original input due to invalid type`);
    ...
    console.log(`stripProtocol is returning ${processed}`);
