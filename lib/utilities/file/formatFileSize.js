...
function formatFileSize(bytes) {
  console.log(`formatFileSize is running with ${bytes} bytes`);
  logger.debug(`formatFileSize formatting file size`, { bytes });
  
  try {
    // Handle invalid inputs gracefully
    if (typeof bytes !== 'number' || isNaN(bytes) || bytes < 0) {
      console.log('formatFileSize received invalid input, returning "0 B"');
    ...
    // Handle zero bytes
    if (bytes === 0) {
      console.log('formatFileSize returning "0 B" for zero bytes');
    ...
    console.log(`formatFileSize returning "${result}"`);
    logger.debug(`formatFileSize formatted successfully`, { input: bytes, output: result });
    return result;

  } catch (error) {
    console.error('formatFileSize encountered unexpected error:', error);
