 ...
 */
function validateConfigValue(value, schema) { // validate config value against schema
  if (!schema || typeof schema !== 'object') { // no schema means valid
    return { isValid: true };
  }

  if (schema.required && (value === undefined || value === null || value === '')) { // check required
    return { isValid: false, error: 'Value is required' };
  }

  if (value === undefined || value === null) { // non-required undefined is valid
    return { isValid: true };
  }
