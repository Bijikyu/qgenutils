...
describe('GitHub Validation Utilities', () => {
  describe('validateGitHubUrl', () => {
    ...
    test('should reject URLs with additional paths', () => {
      ...
      expect(validateGitHubUrl('https://github.com/user')).toBe(errorMsg);
    });

    test('should handle malformed input safely', () => {
      expect(validateGitHubUrl('https://github.com/')).toContain('Please enter a valid GitHub repository URL`);
      expect(validateGitHubUrl('https://github.com/user')).toContain('Please enter a valid GitHub repository URL`);
      expect(validateGitHubUrl('https://github.com//repo')).toContain('Please enter a valid GitHub repository URL`);
    });
  });
  ...
  describe('validateGitHubUrlDetailed', () => {
    test('should return valid result for correct URLs', () => {
      const result = validateGitHubUrlDetailed('https://github.com/user/repo`);
    ...
    test('should categorize empty URL errors', () => {
      const result = validateGitHubUrlDetailed('`);
      expect(result).toMatchObject({
        isValid: false,
        category: 'empty',
    ...
    test('should categorize format errors', () => {
      const result = validateGitHubUrlDetailed('not-a-url`);
      expect(result).toMatchObject({
        isValid: false,
        category: 'format',
    ...
    test('should categorize protocol errors', () => {
      const result = validateGitHubUrlDetailed('http://github.com/user/repo`);
      expect(result).toMatchObject({
        isValid: false,
        category: 'protocol',
    ...
    test('should categorize domain errors', () => {
      const result = validateGitHubUrlDetailed('https://gitlab.com/user/repo`);
      expect(result).toMatchObject({
        isValid: false,
        category: 'domain',
    ...
    test('should categorize path errors', () => {
      const result1 = validateGitHubUrlDetailed('https://github.com/user`);
      ...
      const result2 = validateGitHubUrlDetailed('https://github.com/user/repo/issues`);
    ...
    test('should include original URL in all results', () => {
      ...
      const result = validateGitHubUrlDetailed(originalUrl);
