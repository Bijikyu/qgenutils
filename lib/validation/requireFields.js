...
function requireFields(data, fields, res) {
  console.log(`requireFields is running with fields: ${fields.join(', ')}`);
  logger.debug(`requireFields validating required fields`, { fields });
  
  try {
    // Handle edge cases gracefully
    if (!data || typeof data !== 'object') {
      console.log('requireFields received invalid data object');
    ...
    if (!Array.isArray(fields) || fields.length === 0) {
      console.log('requireFields received invalid fields array');
    ...
    if (missingFields.length === 0) {
      console.log('requireFields validation passed for all fields');
    ...
    console.log(`requireFields validation failed: ${errorMessage}`);
    ...
    console.error('requireFields encountered unexpected error:', error);
