...
function sanitizeString(input) {
  if (!input || typeof input !== 'string') {
    qerrors('sanitizeString called with invalid input', 'sanitizeString', { 
  ...
  if (originalLength !== sanitizedLength) {
    qerrors('String sanitization performed', 'sanitizeString', {
      originalLength,
      sanitizedLength,
...
function sanitizeErrorMessage(error) {
  ...
  // Generic fallback for unrecognized error types
  qerrors('Unknown error type passed to sanitizeErrorMessage', 'sanitizeErrorMessage', {
...
function sanitizeForHtml(input) {
  ...
  qerrors('HTML sanitization performed', 'sanitizeForHtml', {
...
function validatePagination(query, limits = {}) {
  ...
  if (!query || typeof query !== 'object') {
    qerrors('Invalid query object passed to validatePagination', 'validatePagination', {
  ...
  if (limitParam) {
    ...
    if (isNaN(parsedLimit) || parsedLimit < minLimit || parsedLimit > maxLimit) {
      qerrors('Invalid limit parameter in pagination', 'validatePagination', {
  ...
  if (offsetParam) {
    ...
    if (isNaN(parsedOffset) || parsedOffset < 0) {
      qerrors('Invalid offset parameter in pagination', 'validatePagination', {
