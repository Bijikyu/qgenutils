...
function logAuthOperation(functionName, input, result) {
  console.log(`${functionName} is running with ${input || 'none'}`); // log inputs for debugging context
  console.log(`${functionName} is returning ${result}`); // log outputs to trace decision making
}
...
function hasGithubStrategy() {
  try { // attempt strategy detection
    ...
    console.log('hasGithubStrategy has run resulting in a final value of failure'); // manual error indicator
