...
function checkPassportAuth(req) {
  console.log(`checkPassportAuth is running with ${req.user ? req.user.username : 'guest'}`); // Log with user context for security auditing
  try {
    ...
    console.log(`checkPassportAuth is returning ${isAuthenticated}`); // Log authentication result for security monitoring
...
function hasGithubStrategy() {
  ...
  try {
    // Check if Passport is available globally and has GitHub strategy configured
    // passport._strategies is Passport's internal registry of configured authentication strategies
    const configured = Boolean(passport._strategies && passport._strategies.github);
    console.log(`hasGithubStrategy is returning ${configured}`); // Log boolean result for configuration debugging
    return configured;
  } catch (error) {
    // Handle cases where Passport isn't available or configured
    // This is common in applications that don't use authentication or use different auth systems
    qerrors(error, 'hasGithubStrategy'); // Log error for debugging configuration issues
    return false; // Default to strategy not available for graceful degradation
  }
}
