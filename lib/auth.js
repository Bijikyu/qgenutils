...
const { qerrors } = require('qerrors'); // error tracking tool
...
function checkPassportAuth(req) {
  try {
    ...
    // The double-negation (!!) ensures we return a strict boolean rather than truthy/falsy values
    const isAuthenticated = !!(req.isAuthenticated && req.isAuthenticated());
    
    // Log authentication attempt with user context for security auditing
    // Using optional chaining (?.) to safely access potentially undefined user properties
    ...
    // Common scenarios: Passport not configured, malformed request objects, missing middleware
    qerrors(error, 'checkPassportAuth', req); // Log error with full request context for debugging
    
    // SECURITY CRITICAL: Default to unauthenticated state for security (fail-closed approach)
    // Never assume authentication succeeded when error conditions occur
    return false;
  }
}
