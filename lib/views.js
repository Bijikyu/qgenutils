...
function renderView(viewName, data, res) {
  ...
  try {
    // Attempt to render the EJS template with any provided data
    // res.render() is provided by Express when EJS is configured as the view engine
    if (res && typeof res.render === 'function') {
      res.render(viewName, data || {});
    } else {
      throw new Error('res.render is not a function');
    }
 ...
 */
function registerViewRoute(app, routePath, viewName, data) {
  ...
  try {
    ...
    // Register GET route handler that renders the specified template
    // The handler uses our renderView function to get consistent error handling
    app.get(routePath, (req, res) => {
      // Use the renderView utility for consistent error handling
    ...
    // Handle route registration errors (app not available, duplicate routes, etc.)
    qerrors(error, 'registerViewRoute', { routePath, viewName }); // Log error with route context
  }
}
