...
function renderView(viewName, data, res) {
  ...
  try {
    // Attempt to render the requested EJS template with data
    // Express will look for viewName.ejs in the configured views directory
...
function registerViewRoute(app, routePath, viewName, data) {
  ...
  try {
    ...
    app.get(routePath, (req, res) => renderView(viewName, data, res));
