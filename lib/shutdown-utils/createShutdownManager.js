...
function createShutdownManager({ timeout = 30000, exitOnTimeout = true } = {}) {
  ...
  function addHandler(name, handler, priority = 5) {
    ...
    console.log(`Registering shutdown handler: ${name} (priority: ${priority})`);
  ...
  async function executeHandlers(signal) {
    ...
    console.log(`Executing ${handlers.length} shutdown handlers in priority order`);
    ...
    for (const { name, handler, priority } of handlers) {
      ...
      try {
        console.log(`Executing handler: ${name} (priority: ${priority})`);
        ...
        console.log(`Handler completed: ${name} (${duration}ms)`);
        
        results.push({ name, success: true, duration, error: null });
        
      } catch (error) {
        const duration = Date.now() - startTime;
        console.error(`Handler failed: ${name} (${duration}ms) -`, error.message);
  ...
  async function trigger(signal = 'MANUAL') {
    ...
    shutdownPromise = (async () => {
      ...
      try {
        ...
        console.log(`Graceful shutdown completed in ${totalDuration}ms`);
        console.log(`Handlers: ${successCount} successful, ${failureCount} failed`);

        return { success: true, duration: totalDuration, results };

      } catch (error) {
        const totalDuration = Date.now() - shutdownStart;
        console.error(`Shutdown failed after ${totalDuration}ms:`, error.message);
      ...
      if (!registeredSignals.has(signal)) {
        process.on(signal, () => {
          ...
          trigger(signal).then((result) => {
            if (result.success) {
              console.log('Shutdown completed successfully, exiting process');
              process.exit(0);
            } else {
              console.error('Shutdown completed with errors, exiting process');
              process.exit(1);
            }
          });
        });
   ...
   */
  function destroy() {
    console.log('Destroying shutdown manager');
