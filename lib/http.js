...
function calculateContentLength(body) {
  ...
  try {
    // Handle undefined input gracefully - return 0 length for missing content
    if (body === undefined) {
      console.log(`calculateContentLength is returning 0`); // Log fallback for undefined input
      return 0; // Treat undefined as empty content with zero length
    }
