...
function calculateContentLength(body) {
  console.log(`calculateContentLength is running with ${body}`); // record body for troubleshooting
  try {
    ...
    const emptyObj = typeof body === 'object' && body !== null && Object.keys(body).length === 0; // detect {}

    // Handle all valid "empty" body types - these should return '0'
    if (body === null || body === '' || emptyObj) {
      console.log(`calculateContentLength is returning 0`); // note empty body response
      return '0'; // zero length for empty payloads
    }

    // Handle string bodies - most common case for simple content
    if (typeof body === 'string') {
      const len = Buffer.byteLength(body, 'utf8'); // accurate byte size for text
      console.log(`calculateContentLength is returning ${len}`); // record calculated size
      return len.toString(); // header value must be string
    }
    ...
    if (Buffer.isBuffer(body)) { // check for Node Buffer
      ...
      console.log(`calculateContentLength is returning ${len}`); // record buffer size
      return len.toString(); // header value must be string
    }

    // Handle object bodies - JSON APIs
    if (typeof body === 'object') {
      const jsonString = JSON.stringify(body); // convert object for measurement
      const len = Buffer.byteLength(jsonString, 'utf8'); // compute byte length
      console.log(`calculateContentLength is returning ${len}`); // record JSON byte size
      return len.toString(); // header value must be string
    }

    // Fallback for unknown types (numbers, booleans, etc.)
    // These shouldn't occur in typical HTTP usage but we handle gracefully
    console.log(`calculateContentLength is returning 0`); // default for unsupported body
    return '0'; // treat unsupported types as empty
  } catch (error) {
    // Log error with context for debugging
    qerrors(error, 'calculateContentLength', { body }); // structured error log
...
function buildCleanHeaders(headers, method, body) {
    ...
    console.log(`buildCleanHeaders is running with ${safeMethod}`); // log method after normalization
    try {
        ...
        HEADERS_TO_REMOVE.forEach(header => {
            delete cleanHeaders[header]; // strip unwanted header
        });
        ...
        if (emptyObj || emptyBuf) { delete cleanHeaders['content-length']; } // no length for empty payloads
        if (!body || safeMethod === 'get') { delete cleanHeaders['content-length']; } // no length on GET
        ...
        if (safeMethod !== 'get' && body && !emptyObj && !emptyBuf) {
            ...
            if (typeof body === 'string' && body.length > 0) {
                cleanHeaders['content-length'] = calculateContentLength(body); // set for text body
            }
            // Handle object bodies (JSON APIs) - check if object has properties
                ...
                cleanHeaders['content-length'] = calculateContentLength(body); // set for JSON body
            }
            // Handle Buffer bodies with actual content
            else if (Buffer.isBuffer(body) && body.length > 0) {
                cleanHeaders['content-length'] = calculateContentLength(body); // set for Buffer body
            }
        }

        console.log(`buildCleanHeaders is returning ${JSON.stringify(cleanHeaders)}`); // show sanitized header set
        return cleanHeaders;
    } catch (error) {
        // Log error but return original headers as fallback
        // This prevents complete request failure due to header cleaning issues
        qerrors(error, 'buildCleanHeaders', { method: safeMethod }); // capture error details
...
function getRequiredHeader(req, res, headerName, statusCode, errorMessage) {
  const normalizedName = typeof headerName === 'string' ? headerName.toLowerCase() : ''; // ensure consistent case
  console.log(`getRequiredHeader is running with ${normalizedName}`); // track header lookup
  try {
    ...
    if (!headerValue) {
      ...
      console.log(`getRequiredHeader is returning null due to missing header`); // inform debug logs
      return null; // Signal to caller that header was missing
    }

    console.log(`getRequiredHeader is returning ${headerValue}`); // confirm found header
    return headerValue;
  } catch (error) {
    // Handle unexpected errors in header processing
    qerrors(error, 'getRequiredHeader', { headerName: normalizedName, statusCode, errorMessage }); // capture context for debugging
