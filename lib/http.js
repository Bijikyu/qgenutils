...
const { isValidObject } = require('./input-validation');
const { sendJsonResponse, sendAuthError, sendServerError } = require('./response-utils');
...
function calculateContentLength(body) {
  console.log(`calculateContentLength is running with ${body}`); // capture value for which length is being calculated
  try {
    ...
    const emptyObj = typeof body === 'object' && body !== null && Object.keys(body).length === 0; // flag {} as empty

    // Handle all valid "empty" body types - these should return '0'
    if (body === null || body === '' || emptyObj) {
      console.log(`calculateContentLength is returning 0`); // no bytes for null/empty
      return '0'; // HTTP spec expects string value
    }

    // Handle string bodies - most common case for simple content
    if (typeof body === 'string') {
      const len = Buffer.byteLength(body, 'utf8'); // compute byte size of string
      console.log(`calculateContentLength is returning ${len}`); // trace computed length
      return len.toString(); // header values are strings
    }

    // Handle object bodies - JSON APIs
    if (typeof body === 'object') {
      const jsonString = JSON.stringify(body); // convert object to JSON text
      const len = Buffer.byteLength(jsonString, 'utf8'); // measure resulting byte size
      console.log(`calculateContentLength is returning ${len}`); // output byte count
      return len.toString(); // convert to header format
    }

    // Fallback for unknown types (numbers, booleans, etc.)
    // These shouldn't occur in typical HTTP usage but we handle gracefully
    console.log(`calculateContentLength is returning 0`); // unknown types considered empty
    return '0'; // fallback for unsupported body types
  } catch (error) {
    // Log error with context for debugging
    qerrors(error, 'calculateContentLength', { body }); // capture context for devs
...
function buildCleanHeaders(headers, method, body) {
    console.log(`buildCleanHeaders is running with ${method}`); // note HTTP method being processed
    try {
        ...
        const cleanHeaders = { ...headers }; // shallow clone keeps original immutable

        // Remove all blacklisted headers that shouldn't be forwarded
        HEADERS_TO_REMOVE.forEach(header => { delete cleanHeaders[header]; }); // strip sensitive or proxy-only headers
        ...
        if (!body || method.toLowerCase() === 'get') {
            delete cleanHeaders['content-length']; // remove incorrect length on GET or empty body
        }
        ...
        if (method.toLowerCase() !== 'get' && body && Object.keys(body).length > 0) {
            cleanHeaders['content-length'] = calculateContentLength(body); // recompute accurate length for payload
        }

        console.log(`buildCleanHeaders is returning ${JSON.stringify(cleanHeaders)}`); // show final header set
        return cleanHeaders; // provide sanitized headers to caller
    } catch (error) {
        // Log error but return original headers as fallback
        // This prevents complete request failure due to header cleaning issues
        qerrors(error, 'buildCleanHeaders', { method }); // log details for debugging
...
function getRequiredHeader(req, res, headerName, statusCode, errorMessage) {
  console.log(`getRequiredHeader is running with ${headerName}`); // track which header we're retrieving
  try {
    ...
    const headerValue = req?.headers?.[headerName]; // optional chaining avoids TypeErrors

    if (!headerValue) {
      // Send appropriate error response based on status code
      if (statusCode === 401) {
        sendAuthError(res, errorMessage); // delegate 401 logic to helper
      } else {
        sendJsonResponse(res, statusCode, { error: errorMessage }); // generic error response
      }
      console.log(`getRequiredHeader is returning null due to missing header`); // record failure case
      return null; // Signal to caller that header was missing
    }

    console.log(`getRequiredHeader is returning ${headerValue}`); // show value for debugging (avoid in prod)
    return headerValue; // pass header back to caller
  } catch (error) {
    // Handle unexpected errors in header processing
    qerrors(error, 'getRequiredHeader', { headerName, statusCode, errorMessage }); // log details for diagnostics
    sendServerError(res, 'Internal server error', error, 'getRequiredHeader'); // generic server error to client
