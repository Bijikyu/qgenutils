...
function extractGitHubInfo(validatedUrl) {
  ...
  try {
    ...
    if (pathParts.length >= 2) {
      ...
      qerrors(new Error('GitHub info extraction succeeded'), 'extractGitHubInfo', {
        validatedUrl,
        owner,
        repo
      });
    ...
    qerrors(new Error('GitHub info extraction failed - insufficient path parts'), 'extractGitHubInfo', {
      validatedUrl,
...
function validateGitHubRepo(repo) {
  ...
  if (!repo || typeof repo !== 'string') {
    ...
    qerrors(new Error('GitHub repo validation failed - invalid input type'), 'validateGitHubRepo', {
      repo,
  ...
  if (!sanitized) {
    ...
    console.log(`validateGitHubRepo is throwing error: ${errorMsg}`);
    qerrors(new Error('GitHub repo validation failed - empty after sanitization'), 'validateGitHubRepo', {
      originalRepo: repo,
      sanitized,
      errorMsg
    });
  ...
  if (!githubRepoPattern.test(sanitized)) {
    ...
    console.log(`validateGitHubRepo is throwing error: ${errorMsg}`);
    qerrors(new Error('GitHub repo validation failed - invalid format'), 'validateGitHubRepo', {
      sanitized,
      errorMsg,
  ...
  qerrors(new Error('GitHub repo validation succeeded'), 'validateGitHubRepo', {
    originalRepo: repo,
    sanitized
  });
...
function validateGitHubUrlDetailed(url) {
  ...
  if (!sanitizedUrl.trim()) {
    ...
    qerrors(new Error('Detailed GitHub URL validation - empty'), 'validateGitHubUrlDetailed', result);
    return result;
  }
  ...
  try {
    ...
    console.log(`validateGitHubUrlDetailed is returning:`, result);
    qerrors(new Error('Detailed GitHub URL validation - invalid format'), 'validateGitHubUrlDetailed', { 
      ...result, 
  ...
  if (parsedUrl.protocol !== 'https:') {
    ...
    console.log(`validateGitHubUrlDetailed is returning:`, result);
    qerrors(new Error('Detailed GitHub URL validation - invalid protocol'), 'validateGitHubUrlDetailed', {
      ...result,
  ...
  if (parsedUrl.hostname !== 'github.com') {
    ...
    console.log(`validateGitHubUrlDetailed is returning:`, result);
    qerrors(new Error('Detailed GitHub URL validation - invalid domain'), 'validateGitHubUrlDetailed', {
      ...result,
  ...
  if (pathParts.length !== 2) {
    ...
    console.log(`validateGitHubUrlDetailed is returning:`, result);
    qerrors(new Error('Detailed GitHub URL validation - invalid path'), 'validateGitHubUrlDetailed', {
      ...result,
      pathParts: pathParts.length,
  ...
  if (!namePattern.test(owner) || !namePattern.test(repo)) {
    ...
    console.log(`validateGitHubUrlDetailed is returning:`, result);
    qerrors(new Error('Detailed GitHub URL validation - invalid names'), 'validateGitHubUrlDetailed', {
      ...result,
      owner,
      repo,
  ...
  console.log(`validateGitHubUrlDetailed is returning:`, result);
  qerrors(new Error('Detailed GitHub URL validation - success'), 'validateGitHubUrlDetailed', result);
