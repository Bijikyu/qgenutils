...
const { isValidObject, isValidExpressResponse } = require('./input-validation');
...
function sendJsonResponse(res, statusCode, data) {
  console.log(`sendJsonResponse is sending ${statusCode} with`, data); // note outgoing status and payload
  
  // Validate response object before attempting to use it
  if (!isValidExpressResponse(res)) {
    qerrors(new Error('Invalid Express response object'), 'sendJsonResponse', { statusCode, data }); // warn about bad res object
    return; // Cannot send response if res object is invalid
  }
  
  try {
    res.status(statusCode).json(data); // standard Express send pattern
  } catch (error) {
    qerrors(error, 'sendJsonResponse', { statusCode, data }); // log serialization failure
    // If JSON serialization fails, try to send a basic error response
    try {
      res.status(500).json({ error: 'Response serialization failed' }); // attempt fallback minimal response
    } catch (fallbackError) {
      qerrors(fallbackError, 'sendJsonResponse_fallback', { statusCode, data }); // log if fallback also fails
    }
  }
}
...
function sendValidationError(res, message, additionalData = {}, statusCode = 400) {
  console.log(`sendValidationError is sending ${statusCode} with message: ${message}`); // announce validation failure response
  
  if (!isValidExpressResponse(res)) {
    qerrors(new Error('Invalid Express response object'), 'sendValidationError', { message, additionalData, statusCode }); // report bad res instance
    return; // abort sending if response object invalid
  }
  
  const errorResponse = { error: message, ...additionalData }; // create consistent error payload
  sendJsonResponse(res, statusCode, errorResponse); // delegate sending to generic helper
}
...
function sendAuthError(res, message = 'Authentication required') {
  console.log(`sendAuthError is sending 401 with message: ${message}`); // announce auth failure
  sendJsonResponse(res, 401, { error: message }); // standardized 401 body
}
 ...
 * 
 * @param {object} res - Express response object
 * @param {string} message - Public error message (defaults to generic message)
...
function sendServerError(res, message = 'Internal server error', error = null, context = null) {
  console.log(`sendServerError is sending 500 with message: ${message}`); // log context about server failure
  
  // Log detailed error information for debugging
  if (error) {
    qerrors(error, 'sendServerError', { message, context }); // capture stack trace for debugging
  }

  sendJsonResponse(res, 500, { error: message }); // deliver generic server error
}
