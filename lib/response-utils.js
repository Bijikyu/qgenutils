...
function sendAuthError(res, message = 'Authentication required') {
  console.log(`sendAuthError is sending 401 with message: ${message}`);
...
function sendServerError(res, message = 'Internal server error', error = null, context = null) {
  console.log(`sendServerError is sending 500 with message: ${message}`);
  
  // Log detailed error information for debugging
  if (error) {
    qerrors(error, 'sendServerError', { message, context });
  }
